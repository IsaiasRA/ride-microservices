...
from app1.auth import (gerar_tokens,
                        rota_protegida,
                         validar_token)
from test.test_auth import (refresh_valido, criar_usuario,
                            salvar_refresh, revogar_refresh,
...
def buscar_passageiro(id):
    try:
        ...
        with conexao() as cursor:
            cursor.execute('SELECT * FROM passageiros WHERE id = ?', (id,))
...
def register():
    try:
        ...
        with conexao() as cursor:
            cursor.execute('SELECT id FROM usuarios WHERE usuario = ?',
                           (dados['usuario'],))
...
def atualizar_passageiro(id):
    try:
        ...
        set_sql = ", ".join(f"{campo} = ?" for campo in enviados.keys())
        valores = list(enviados.values())
        valores.append(id)

        query = f"UPDATE passageiros SET {set_sql} WHERE id = ?"
        with conexao() as cursor:
...
def deletar_passageiro(id):
    try:
        ...
        with conexao() as cursor:
            cursor.execute('DELETE FROM passageiros WHERE id = ?', (id,))
            if cursor.rowcount == 0:
                logger.warning(f'Passageiro {id} não encontrado.')
                return jsonify({'erro': 'Passageiro não encontrado!'}), 404
...
def buscar_motorista(id):
    try:
        ...
        with conexao() as cursor:
            cursor.execute('SELECT * FROM motoristas WHERE id = ?', (id,))
            dado = cursor.fetchone()
...
def atualizar_motorista(id):
    try:
        ...
        query = f"UPDATE motoristas SET {set_sql} WHERE id = ?"
        with conexao() as cursor:
            cursor.execute(query, valores)
...
def adicionar_viagem():
    try:
        ...
        with conexao() as cursor:
            cursor.execute('SELECT valor FROM passageiros WHERE id = ?',
                           (dados['id_passageiro'],))
            ...
            cursor.execute('SELECT * FROM motoristas WHERE id = ?',
                           (dados['id_motorista'],))
            ...
            cursor.execute('UPDATE passageiros SET valor = valor - ? WHERE id = ?',
                           (total_viagem, dados['id_passageiro']))

            cursor.execute('UPDATE motoristas SET quantia = quantia + ? WHERE id = ?',
    ...
    try:
        ...
        with conexao() as cursor:
            ...
            if viagem[3] == 'cancelada' or viagem[4] == 'cancelado':
                cursor.execute(
                    'SELECT valor FROM passageiros WHERE id = ?',
                               (viagem[0],))
                ...
                cursor.execute(
                    'SELECT quantia FROM motoristas WHERE id = ?',
                               (viagem[1],))
                ...
                cursor.execute(
                    'UPDATE motoristas SET quantia = quantia - ? WHERE id = ?',
                               (viagem[2], viagem[1]))

                cursor.execute(
                    'UPDATE passageiros SET valor = valor + ? WHERE id = ?',
                               (viagem[2], viagem[0]))
            ...
            query = f"UPDATE viagens SET {set_sql} WHERE id = ?"
            cursor.execute(query, valores)
    ...
    try:
        ...
        with conexao() as cursor:
            cursor.execute('''
                    ...
                    total_viagem FROM viagens WHERE id = ?
                    ''', (id,))
            viagem = cursor.fetchone()
            ...
            cursor.execute('SELECT quantia FROM motoristas WHERE id = ?',
                           (viagem[1],))
...
def buscar_registro_pagamento(id):
    try:
        ...
        with conexao() as cursor:
            cursor.execute('SELECT * FROM registros_pagamento WHERE id = ?', (id,))
            dado = cursor.fetchone()
        ...
        with conexao() as cursor:
            ...
            if viagem[3] == 'cancelada' or viagem[4] == 'cancelado':
                cursor.execute('''
                        ...
                        pagamento = 'cancelado', status = 'cancelado' WHERE id = ?''',
                        (id,))
                ...
                cursor.execute('DELETE FROM viagens WHERE id = ?', (registro[0],))
            ...
            query = f'UPDATE registros_pagamento SET {set_sql} WHERE id = ?'
            cursor.execute(query, valores)
...
def deletar_registro_pagamento(id):
    try:
        ...
        with conexao() as cursor:
            cursor.execute(
                'DELETE FROM registros_pagamento WHERE id = ?', (id,))
